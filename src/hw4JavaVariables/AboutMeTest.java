package hw4JavaVariables;

public class AboutMeTest {
	public static void main(String[]args) {
		AboutMe aboutme = new AboutMe();
	    aboutme.name = "Md Khalequzzman"; // Variables initialized
	    aboutme.age = 29;
	    aboutme.myApartmentRent = 1000;
	    aboutme.myYearlySalary = 80000;
	    aboutme.myBankBalance = 30000l;
	    aboutme.myHeight = 1.9f;
	    aboutme.myGrade = 3.6;
	    aboutme.sex = 'M';
	    aboutme.usCitizen = false;		
	    aboutme.aboutMe(); // method initialized
	    
	    AboutMe aboutme1 = new AboutMe();
	    aboutme.name = "alex"; // Variables initialized
	    aboutme.age = 30;
	    aboutme.myApartmentRent = 1500;
	    aboutme.myYearlySalary = 70000;
	    aboutme.myBankBalance = 20000l;
	    aboutme.myHeight = 1.7f;
	    aboutme.myGrade = 3.6;
	    aboutme.sex = 'M';
	    aboutme.usCitizen = false;		
	    aboutme.aboutMe(); // method initialized
	    
	    

	}

}
